...
######################################################
#              BuildX - STM32F4XX Driver             #
#----------------------------------------------------#
# File    : buildx.mk                                #
# Version : 1.0.0                                    #
# Desc    : For building the STM32F4xx_HAL_Driver.   #
#----------------------------------------------------#
# History)                                           #
...
module_dir_output_inc  = $(module_dir_output_base)/include/stm32f4drv
...
module_build_cflags    = \
	-I$(current_dir_abs)/Inc \
	-I$(basedir)/Drivers/CMSIS/Include \
	-I$(basedir)/Drivers/CMSIS/Device/ST/STM32F4xx/Include \
	-I$(basedir)/Inc
module_build_ldflags   = 
module_build_target_a  = $(build_opt_a_pre)stm32f4drv.$(build_opt_a_ext)
module_build_target_so = $(build_opt_so_pre)stm32f4drv.$(build_opt_so_ext)
...
ifeq ($(build_run_so),1)
module_objs_so_tmp0    =
module_objs_shared     = $(patsubst %.c,%.lo,$(module_build_src_mk))
...
post:
	...
	@echo "----------------------------------------------------------------"
	$(CP) $(current_dir_abs)/Inc/*.h $(module_dir_output_inc)
