...
static __IO int32_t g_diff_sec = 0L;
static __IO int32_t g_tv_sec = 0L;
static __IO int32_t g_tv_milli = 0L;
...
static int32_t stx_gmtime(register int32_t ntt, stx_ts_t *ts);
static int32_t stx_mktime(register stx_ts_t *ts);
static int32_t /*@alt void@*/stx_gettime(int32_t clockId, tspec_t *ts);
static int32_t stx_settime(int32_t clockId, const tspec_t *ts);
...
int32_t stx_clock_sec2ts(int32_t sec, stx_ts_t *ts) {
	int32_t rc;

	rc = stx_gmtime(sec, ts);
	if (rc == 0L) {
		ts->msec = 0;
		ts->tz = TZ_KST;
	}
...
int32_t stx_clock_ts(stx_ts_t *ts) {
	tspec_t now;
	int32_t rc;

	stx_gettime(CLOCK_REALTIME, &now);

	rc = stx_gmtime(now.sec, ts);
	if (rc == 0L) {
		ts->msec = now.milli;
		ts->tz = TZ_KST;
	}
...
int32_t stx_clock_set(stx_ts_t *ts) {
	tspec_t now;

	now.sec = stx_clock_ts2sec(ts);
	now.milli = ts->msec;
	...
	return stx_settime(CLOCK_REALTIME, &now);
}

void stx_clock_ts_print(const stx_ts_t *ts) {
	util_print("[TimeStamp] %04d-%02d-%02d %02d:%02d:%02d.%03d\r\n", ts->year,
